...
class Dijkstra :
    ...
    def add_cross_point(self,voyage_no_start_df, voyage_no_dest_df):
        ...
        while i < len(voyage_no_start_df) - 1:
            start_lat1 = voyage_no_start_df.iloc[i, 8]  # 위도
            start_long1 = voyage_no_start_df.iloc[i, 9]  # 경도
            start_lat2 = voyage_no_start_df.iloc[i + 1, 8]  # 위도
            start_long2 = voyage_no_start_df.iloc[i + 1, 9]  # 경도
            p1 = (start_lat1, start_long1)
            p2 = (start_lat2, start_long2)
            # self.graph.add_node(start)
            j = 0
            while j < len(voyage_no_dest_df) - 1:
                dest_lat1 = voyage_no_dest_df.iloc[j, 8]  # 위도
                dest_long1 = voyage_no_dest_df.iloc[j, 9]  # 경도
                dest_lat2 = voyage_no_dest_df.iloc[j + 1, 8]  # 위도
                dest_long2 = voyage_no_dest_df.iloc[j + 1, 9]  # 경도
    ...
    def make_start_to_end_graph(self,voyage_df):
        ...
        while i < len(voyage_df) - 1:
            start_lat = voyage_df.iloc[i, 8].astype('float')  # 위도
            start_long = voyage_df.iloc[i, 9].astype('float')  # 경도
            start = (start_lat, start_long)
            self.graph.add_node(start)

            dest_lat = voyage_df.iloc[i + 1, 8].astype('float')  # 위도
            dest_long = voyage_df.iloc[i + 1, 9].astype('float')  # 경도
    ...
    """
       ...
       :return:
       """
    def add_cross_point_to_graph(self,start_port,dest_port,csv_name):
    ...
    """
       ...
       :return:
       """
    def create_passage_plan(self,start_port,dest_port,csv_name):
        #voyage_df = self.db.get_voyage_no(start_port, dest_port)  # DB에서 항로번호 조회
        ...
        return passage_plan_dict # 화면에 표시해주기 위해
