...
def main(passage_id):
    ...
    "항로별 노드저장"
    "============================================================================"
    passage_plan_dict ,passage_plan_dict_distance  = dijk.create_passage_plan(start_port,dest_port,csv_name)
    ...
    for passage_plan in final_route:
        temp_coord = '['+str(passage_plan[0])+ ','+ str(passage_plan[1])+']'
        marker_str = 'L.marker([' + str(passage_plan[0])+ ','+ str(passage_plan[1])+ '],{icon: red_pointIcon,title:"'+temp_coord+'"}).bindPopup("'+temp_coord+'").addTo(map).openPopup();'
    ...
    return final_route_text,final_route_marker,passage_plan_dict,barriers_list,final_route_distance,passage_plan_dict_distance
